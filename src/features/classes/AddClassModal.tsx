...
import { createClass, DuplicateNameError } from '../../api/classes';
import { fetchSchools } from '../../api/students';
import { CLASSES_QUERY_KEY } from './queryKeys';

const schema = z.object({
  schoolId: z.string().min(1, 'Select a school'),
  name: z.string().trim().min(1, 'Class name is required').max(100, 'Keep this under 100 characters'),
});
...
function AddClassModal({ open, onClose }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => createClass({ schoolId: values.schoolId, name: values.name.trim() }),
    onSuccess: (created) => {
      message.success(`Class "${created.name}" added`);
    ...
    onError: (error) => {
      ...
      message.error('Could not add the class. Please try again.');
    },
  });
      ...
      <Form layout="vertical" requiredMark="optional" onFinish={submit}>
        ...
        <Controller
          ...
          render={({ field }) => (
            ...
            >
              <Input {...field} placeholder="e.g. Grade 5" autoComplete="off" />
            </Form.Item>
          )}
        />
      </Form>
